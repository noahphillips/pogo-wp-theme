...
gulp.task('editorStyles', () => {
	return gulp
		...
		.pipe(
			sass({
				errLogToConsole: config.editorErrLogToConsole,
				outputStyle: config.editorOutputStyle,
				precision: config.editorPrecision
			})
		)
		.on('error', sass.logError)
		// .pipe(sourcemaps.write({includeContent: false}))
...
gulp.task(
	...
	gulp.parallel('styles', 'editorStyles', 'vendorsJS', 'customJS', 'images', browsersync, () => {
		...
		gulp.watch(config.watchStyles, gulp.parallel('styles', 'editorStyles')); // Reload on SCSS file changes.

...
var fVideo = document.querySelector("video#forward");
var rVideo = document.querySelector("video#reverse");
var dotsContainer = document.querySelector(".dots");
var dots = dotsContainer.querySelectorAll(".dot");
var fTimes = [[9, 12], [15.5, 18], [23, 27]];
var rTimes = [[5, 9.5], [14.5, 17], [21, 23.5]];
var n = 3;

function onLeave(o, d, direction) {
  if (direction === "down") {
    fVideo.classList.add("active");
    rVideo.classList.remove("active");
    rVideo.currentTime = rTimes[n - d.index][0];

    if (Math.abs(o.index - d.index) === 1) {
      playSegment(fVideo, fTimes[o.index]);
    } else {
      fadeBetween(fVideo, rVideo, fTimes[d.index - 1][1]);
    }
  } else if (direction === "up") {
    rVideo.classList.add("active");
    fVideo.classList.remove("active");
    fVideo.currentTime = fTimes[d.index][0];

    if (Math.abs(o.index - d.index) === 1 && o.index !== 3) {
      playSegment(rVideo, rTimes[n - o.index]);
    } else {
      fadeBetween(rVideo, fVideo, rTimes[n - d.index - 1][1]);
    }
  }
...
function afterLoad(o, d, direction) {
  if (d.index === 3) {
...
for (var i = 0; i < dots.length; i++) {
  dots[i].addEventListener("click", function (e) {
    if (!e.currentTarget.classList.contains("active")) {
      fullpage_api.moveTo(Array.prototype.indexOf.call(dots, e.currentTarget) + 1);
    }
  });
}

function playSegment(video, times) {
  freeze();
  video.currentTime = times[0];
  video.addEventListener("timeupdate", function listener() {
    if (video.currentTime >= times[1]) {
      video.pause();
...
function fadeBetween(startVideo, endVideo, svTime) {
  freeze();
  setTimeout(function () {
    startVideo.classList.add("fade");
    endVideo.classList.add("active");
    startVideo.classList.remove("active");
    setTimeout(function () {
      startVideo.currentTime = svTime;
      startVideo.classList.remove("fade");
      unfreeze();
    }, 1500);
  }, 500);
}
...
fVideo.currentTime = 9; // uncomment this to turn off the scrolljacking
// fullpage_api.destroy();
},{"fullpage.js":"../node_modules/fullpage.js/dist/fullpage.js"}],"../node_modules/parcel-bundler/src/builtins/hmr-runtime.js":[function(require,module,exports) {
